...
contract InsuranceContract is ChainlinkClient, ConfirmedOwner {
    ...
    function updateContract() public onContractActive() {
        ...
        if (contractActive) {
            dataRequestsSent = 0;
            string memory url = string(abi.encodePacked("https://worldweatheronline.com/api/v1/weather.ashx?key=", "your_api_key", "&q=", cropLocation, "&format=json&num_of_days=1"));
            checkRainfall(oracles[0], jobIds[0], url, "data.current_condition.0.precipMM");

            url = string(abi.encodePacked("https://api.weatherbit.io/v2.0/current?city=", cropLocation, "&key=", "your_api_key"));
